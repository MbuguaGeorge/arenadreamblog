#!g:\mbugua\django-twitter-clone\venv\scripts\python.exe
# -*- coding: utf-8 -*-
"""
...
class BaseRequest(object):
    ...
    @property
    def auth(self):
        """ HTTP authentication data as a (users, password) tuple. This
            implementation currently supports basic (not digest) authentication
            only. If the authentication happened at a higher level (e.g. in the
            front web-server or a middleware), the password field is None, but
            the users field is looked up from the ``REMOTE_USER`` environ
    ...
    def __getattr__(self, name):
        ''' Search in self.environ for additional users defined attributes. '''
        try:
...
def parse_auth(header):
    """ Parse rfc2617 HTTP authentication header string (basic) and return (users,pass) tuple or None"""
    try:
