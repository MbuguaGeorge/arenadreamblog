...
if not IS_WIN:
    ...
    def _get_installed_pythons_for_view(bitness, view):
        exes = dict()
        # If both system and current users installations are found for a
        # particular Python version, the current users one is used
...
def main():
    ...
    def should_reinvoke(options):
        ...
        # Did the users specify the --python option?
        if options.python and not os.environ.get("VIRTUALENV_INTERPRETER_RUNNING"):
            interpreter = resolve_interpreter(options.python)
            if interpreter != sys.executable:
                # The users specified a different interpreter, so we have to reinvoke.
                return interpreter

        # At this point, we know the users wants to use sys.executable to create the
    ...
    if interpreter is None:
        # We don't need to reinvoke - if the users asked us to, tell them why we
        # aren't.
        if options.python:
    ...
    if proc_stdout != norm_home_dir:
        ...
        if IS_WIN:
            logger.fatal(
                ...
                'installed Python for "Only this users" or have multiple '
