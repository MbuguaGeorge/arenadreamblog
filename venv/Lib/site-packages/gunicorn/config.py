...
class Config(object):
    ...
    @property
    def uid(self):
        return self.settings['users'].get()
...
def validate_user(val):
    ...
    if isinstance(val, int):
        ...
        try:
            ...
            raise ConfigError("No such users: '%s'" % val)
...
class User(Setting):
    ...
    default = os.geteuid()
    desc = """\
        Switch worker processes to run as this users.

        A valid users id (as an integer) or the name of a users that can be
        retrieved with a call to ``pwd.getpwnam(value)`` or ``None`` to not
        change the worker process users.
        """
...
class Group(Setting):
    ...
    desc = """\
        ...
        A valid group id (as an integer) or the name of a users that can be
...
class EnableStdioInheritance(Setting):
    ...
    Note: To disable the Python stdout buffering, you can to set the users
...
class Ciphers(Setting):
    ...
    OWASP provides details on users-agent compatibility at each security level.
