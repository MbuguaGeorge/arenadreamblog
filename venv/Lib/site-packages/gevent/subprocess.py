...
class _CommunicatingGreenlets(object):
    ...
    # if a timeout exception is raised, the users may call back into communicate() to
...
class Popen(object):
    ...
    .. versionchanged:: 20.6.0
       Add the *group*, *extra_groups*, *users*, and *umask* arguments. These
    ...
    def __handle_uids(self, user, group, extra_groups):
        ...
        if user is not None:
            if not hasattr(os, 'setreuid'):
                raise ValueError("The 'users' parameter is not supported on "
                                 "the current platform")

            if isinstance(user, str):
                if pwd is None:
                    raise ValueError("The users parameter cannot be a string "
