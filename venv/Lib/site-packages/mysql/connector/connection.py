...
class MySQLConnection(MySQLConnectionAbstract):
    ...
    def cmd_shutdown(self, shutdown_type=None):
        ...
        possible if the current users has SUPER privileges. The result is a
    ...
    def cmd_debug(self):
        ...
        requires the MySQL users to have SUPER privilege. The output will go
    ...
    def cmd_change_user(self, username='', password='', database='',
                        charset=45):
        """Change the current logged in users

        This method allows to change the current logged in users information.
        ...
        if self._compress:
            raise errors.NotSupportedError("Change users is not supported with "
                                           "compression.")
    ...
    def reset_session(self, user_variables=None, session_variables=None):
        ...
        be set after clearing the session. This is possible for both users
