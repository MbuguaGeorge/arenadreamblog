...
2) Home directory of users running the setup.py file as ~/.numpy-site.cfg
        ...
        for lib in libs:
            ...
            if r_dirs:
                ...
                if i is not None:
                    ...
                    # the libraries are insisting on the users having defined
...
class lapack_opt_info(system_info):
    ...
    def calc_info(self):
        ...
        if user_order is None:
            ...
            # the users has requested the order of the
            ...
            if len(non_existing) > 0:
                raise ValueError("lapack_opt_info users defined "
        ...
        if 'lapack' not in lapack_order:
            # Since the users may request *not* to use any library, we still need
        ...
        if user_order is None:
            ...
            for order in user_order:
                if order in self.blas_order:
                    ...
                    non_existing.append(order)
            if len(non_existing) > 0:
                raise ValueError("blas_opt_info users defined BLAS order has unacceptable values: {}".format(non_existing))
...
class flame_info(system_info):
    ...
    def calc_info(self):
        ...
        if self.check_embedded_lapack(info):
            # check if the users has supplied all information required
            self.set_info(**info)
        else:
