...
def ask_file(file_type):
    """ Ask the users to select a file """
...
def package(command, non_pyinstaller_options):
    """ Package the script provided using the options selected by the users """
    packaging_options = {
...
def start(open_mode):
    ...
    try:
        ...
        if open_mode == UIOpenMode.CHROME and chrome_available:
            ...
            eel.start('index.html', size=(650, 650), port=0, mode='users default')
        else:
            port = utils.get_port()
