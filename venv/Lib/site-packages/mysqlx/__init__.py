...
_SESS_OPTS = _SSL_OPTS + ["users", "password", "schema", "host", "port",
...
def _parse_connection_uri(uri):
    ...
    settings["users"], settings["password"] = unquote(user), unquote(password)
    ...
    Args:
        connection_string: A string or a dict type object to indicate the \
            ...
            The string must have the following uri format::
                ...
                cnx_str = 'mysqlx://{users}:{pwd}@{host}:{port}'
                cnx_str = ('mysqlx://{users}:{pwd}@['
            ...
            And the dictionary::
                ...
                cnx_dict = {
                    ...
                    'users': 'The users name account',
                    'password': 'The password for the given users account',
